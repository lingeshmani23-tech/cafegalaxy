    ...
    >
      ...
      <div className="p-4 flex-grow flex flex-col justify-between relative">
        {!item.image && item.isPopular && (
          <span className="absolute -top-3 left-4 bg-[#FFC107] text-[#0A0A0A] text-[10px] font-extrabold tracking-widest uppercase py-1 px-2.5 rounded-lg flex items-center gap-1 shadow-sm">
            <Award size={10} /> Popular
          </span>
        )}
        
        {!item.image && (
          <div className="absolute top-4 right-4 flex items-center justify-center bg-white/10 backdrop-blur-sm p-1 rounded-md border border-gray-700 shadow-sm">
            <div
        ...
        <div>
          <div className={`flex justify-between items-start gap-2 mb-1.5 ${!item.image && item.isPopular ? 'mt-3' : ''}`}>
