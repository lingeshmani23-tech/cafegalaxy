...
for (const [category, items] of Object.entries(rawMenu)) {
  for (const item of items) {
    ...
    // Hyper-realistic food delivery app style prompt
    const query = encodeURIComponent(`Simple clean realistic food photography of ${name}, white background, swiggy zomato delivery app style, high quality real food photograph, no text`);
    const imageUrl = `https://image.pollinations.ai/prompt/${query}?width=250&height=250&nologo=true&model=flux`;
...
console.log("menuData.js rebuilt successfully with " + menuItems.length + " items using fast-loading minimalistic photos!");
