...
for (const [category, items] of Object.entries(rawMenu)) {
  for (const item of items) {
    ...
    // Generate an image url with pollinations AI using an ultra-realistic prompt
    const query = encodeURIComponent(`Professional ${name} photography authentic DSLR food photography shallow depth of field white background no text 8k`);
    const imageUrl = `https://image.pollinations.ai/prompt/${query}?width=800&height=600&nologo=true`;
...
console.log("menuData.js rebuilt successfully with " + menuItems.length + " items!");
