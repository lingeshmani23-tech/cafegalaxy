...
const dataPath = path.join('d:', 'Project', 'Cafe Galaxy', 'src', 'data', 'menuData.js');
let data = fs.readFileSync(dataPath, 'utf8');

// The file exports menuCategories and menuItems.
// I will parse the menuItems, update their images, and rewrite the file.
...
// We will do a regex replacement on image urls
for (const [name, query] of Object.entries(queries)) {
    // Find the object block that has name: "Veg Sandwich"
    const nameRegex = new RegExp("name:\\\\s*\\"" + name + "\\"[\\\\s\\\\S]*?image:\\\\s*\\"[^\\"]*\\"", 'g');
    data = data.replace(nameRegex, (match) => {
        const imageUrl = "https://image.pollinations.ai/prompt/" + encodeURIComponent(query + " authentic DSLR food photography shallow depth of field white background no text 8k") + "?width=800&height=600&nologo=true";
