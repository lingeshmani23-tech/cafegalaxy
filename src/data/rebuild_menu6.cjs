...
// Map exact user requested search term
const searchTerms = {
  // Sandwiches
  "Veg Sandwich": "Fresh Veg Sandwich",
  "Potato Tikki Sandwich": "Aloo Tikki Sandwich",
  "Grilled Veg Sandwich": "Grilled Vegetable Sandwich",
  "Grilled Veg Cheese Sandwich": "Grilled Cheese Vegetable Sandwich",
  ...
  "Smiles": "Potato Smiley Fries",
  "Potato Bites": "Crispy Potato Bites",
  ...
  "Blu Bleed Mojito": "Blue Curacao Mojito",
...
for (const [category, items] of Object.entries(rawMenu)) {
  for (const item of items) {
    ...
    // Get the exact user requested search term
    const exactSearchTerm = searchTerms[name] || name;
    
    // Convert to comma separated keywords for loremflickr
    const keywords = encodeURIComponent(exactSearchTerm.split(' ').join(','));
    
    // Generate unique seed for lock parameter so images stay consistent
    const uniqueLock = Math.floor(Math.random() * 10000) + idCounter;
    
    const imageUrl = `https://loremflickr.com/500/500/${keywords},food/all?lock=${uniqueLock}`;
...
console.log("menuData.js rebuilt successfully with " + menuItems.length + " exact LoremFlickr images!");
