...
async function scrapeUnsplash() {
  ...
  const browser = await puppeteer.launch({ 
    headless: "new",
    executablePath: "C:/Users/linge/.cache/puppeteer/chrome/win64-151.0.7922.47/chrome-win64/chrome.exe"
  });
  ...
  // Loop through a few items first to test logic, or just run all
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      ...
      await page.waitForSelector('figure img, img[srcset]', { timeout: 5000 }).catch(() => {});
      
      // Extract the best image source
      const imgUrl = await page.evaluate(() => {
        // Find the first image in the main grid
        const imgs = Array.from(document.querySelectorAll('img[srcset], figure img'));
        for (const img of imgs) {
          // Avoid avatars or icons
...
scrapeUnsplash().catch(console.error);
