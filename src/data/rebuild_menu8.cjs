...
for (const [category, items] of Object.entries(rawMenu)) {
  for (const item of items) {
    ...
    // Map to local images downloaded from AI
    const itemId = `menu_${idCounter++}`;
    const imageUrl = `/images/menu/${itemId}.jpg`;
...
console.log("menuData.js rebuilt successfully with local AI images!");
